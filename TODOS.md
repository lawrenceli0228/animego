...
## 分集标题还有 6,013 行没跑（等 dandanplay 当日额度重置）

**What** — 用 `bgmbackfill --heal-episode-titles --apply --resume` 把剩下的 6,013 行跑完。
...
## dandanplay 交叉链接补 bgm_id：形态未定，先跑探针

**What** — 跑 `bgmbackfill --probe-ddp-crosslink --limit <N>`，用真实命中率和每行请求成本，在「`/match` 机会捕获」和「搜索驱动 sweep」之间做决定。
...
**Depends on / blocked by** — 番剧详情组的当日额度；与上一条抢同一份预算。
