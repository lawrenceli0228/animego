...
## 分集标题还有 6,805 行没跑（等 dandanplay 当日额度重置）

**What** — 用 `bgmbackfill --heal-episode-titles --apply --resume` 把剩下的 6,805 行跑完。（2026-09-04 实测数：6,013 涨到 6,805 是因为 id-map sweep 那天又绑上了 791 行，它们随即成为新候选。）

**Why** — 2026-09-03 那轮跑到约 4,700 次请求时番剧详情组当日额度耗尽，跑批被我手动杀掉。`--resume` 的断点是 `episode_titles_at` 戳，跑完的行不会重跑，所以直接续跑即可，不用从头。当天成果已提交：分集标题 202,958 → 207,557 条，有标题的番剧 11,205 → 11,315 部。

**Context** — 跑之前先确认三件事已经在跑的镜像里（都在 `47e5bcd` 之后）：熔断器只由真正走到网络的行驱动（缓存命中中立）、`--limit` 默认 4,500 留一成给线上、`ScopeExceeded` 以 2.00x 拒绝范围不符的写入。**这一轮会开始出现 `SCOPE_MISMATCH` 分类**，那是守卫在工作不是故障。⚠️ 番剧详情组和线上 `/match` 共用额度，别把 `--limit` 设成 0。

**2026-09-04 跑了一轮** — `--limit 4000 --apply --resume`，在预算边界正常收尾，**没有中止、零 FETCH_FAIL、零 QUOTA_SPENT**。新写 17,646 条标题（中文 10,636），新覆盖 1,384 部（6,708 → 8,092），撤回 0。**剩余候选 5,420**，等下一个额度日续跑。分类：SKIPPED 2,706 / NO_TITLES 1,520 / WRITTEN 1,384 / UNDECIDED 756 / DDP_REBIND 416 / SCOPE_MISMATCH 22。★ SCOPE_MISMATCH 首次出现，是守卫在工作。★ 当日番剧详情组用掉约 4,215/5,000（含探针 ~215）。

**2026-09-04 更新** — 两件事变了。**一**：6,805 行 > 5,000 的日额度，所以这是两天的活，中途必然撞到额度墙。**二**：撞墙现在会**干净地停下并说明原因**——客户端能认出配额拒绝了，回填把它单列为 `QUOTA_SPENT` 并在第一次出现时中止（不走熔断器：配额拒绝现在是 error 不是空结果，而 `nextEmptyStreak` 对非 NO_TITLES 一律清零，熔断器再也不会触发）。断点安全：`episode_titles_at` 只在写标题的同一个事务里打戳，没读到的行不打戳、仍是候选。

**Depends on / blocked by** — 只等额度重置（每日）。
