...
## 简繁折叠：搜索仍有约 1,319 行只认自己被存成的那一种字形

- **What:** 2026-08-28 把 `/api/anime/search` 改成本地优先之后，关键词会同时匹配
  `title_chinese` 和 `title_hant` 五列。这覆盖了同时存有两种字形的行 —— 也就是绝大
  多数。剩下的没有覆盖：**57 行有简体无繁体、1,262 行有繁体无简体**（共 1,319 行，
  占 17,689 行的 7.5%）。这些行只答复自己被存成的那一种字形。
- **Why:** 真正的折叠要把**查询词**做字形归一（OpenCC 那类映射），或者给表加一个
  归一化的生成列。前者要给 go-api 引入一个字典依赖，后者要 Postgres 侧有映射函数
  ——两条都不是「顺手加个索引」的量级。
- **Pros:** 现在的实现已经把用户实际会撞到的绝大部分修好了；剩下 7.5% 是长尾。
- **Cons:** 长尾里 1,262 行是「有繁无简」，也就是**简体用户搜不到的那一批**，而简体
  是主要读者群 —— 这个方向的缺口比反方向大 22 倍，不是对称的。
- **Context:** 更便宜的半个解法：给那 1,262 行跑一次 t2s 回填 `title_chinese`
  （`title_hant_source` 已经区分了 wikipedia / anilist / opencc 三种来源，opencc 那
  5,932 行本来就是从简体转出来的，必然已有简体）。那是一次数据回填，不改查询。
- **Depends on / blocked by:** 无。迁移 0027 和本地搜索已经上了，这条是在其之上收窄
  剩余缺口。
