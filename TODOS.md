...
## 简繁折叠：把查询词转一遍，别去回填 `title_chinese`

- **What:** 2026-08-28 把 `/api/anime/search` 改成本地优先之后，关键词同时匹配五个
  标题列，覆盖了同时存有两种字形的行 —— 也就是绝大多数。没覆盖的是
  **57 行有简无繁、1,262 行有繁无简**（共 1,319 行，占 17,689 的 7.5%）。
  做法建议：**在搜索时把查询词用现有的 s2twp 表转一遍，作为额外的匹配模式**，
  不写库、不改显示。
- **Why:** 「有繁无简」不是随机长尾，成因很具体：简体标题的**唯一来源是 Bangumi
  富化**，而这批行的 `bgm_id` 覆盖率只有 **9.4%**（全库 70.8%，差 7.5 倍）——
  没有 bgm 映射就没有条目可读，简体无从产生；与此同时 AniList 的 synonyms 带着
  繁体标题，被 0022 的回填填进了 `title_hant`（1,262 行里 1,248 行来源是 anilist，
  opencc 来源 0 行）。它们的 `bangumi_version` 全是 3 —— **富化跑完了，只是没东西可读**。
  标题一看就明白为什么没映射：几乎全是续作和篇章（「第二季」「The Final Season」
  「千年血戰篇-禍進譚-」「II」），正是标题匹配最难的一格。format 上 `SPECIAL` 中招
  14.1%、`TV` 6.6%，但绝对数里 315 部是 TV，含进击的巨人 / 排球少年 / BLEACH /
  药师少女 / 海盗战记这些主线番，不是冷门条目。
- **Pros（为什么选转查询词而不是回填数据）:** 实测支撑两条 ——
  ①**代价小**：加载 `data/hant/opencc-s2twp.txt` 实测 **23ms / 10.7MB**，一次性；
  目前它只在 river worker 里加载，请求路径上没有，需要在启动时也加载一份。
  ②**真的能对上**：拿这 1,262 行里的真实标题验，用户会敲的简体经 s2twp 转出来的串
  **8/8 逐字等于库里存的 `title_hant`**（海盗战记 第二季 / 药师少女的独语 第二季 /
  进击的巨人 The Final Season / 排球少年!! 乌野高中 VS 白鸟泽学园高中 /
  BLEACH 死神 千年血战篇-祸进谭- 等）。覆盖 1,262/1,319 = **96%**，
  且正好是不对称里大的那一半。
- **Cons（原先设想的 t2s 回填为什么更贵，这条是修正）:** 早先这里写的是
  「给那 1,262 行跑一次 t2s 回填 `title_chinese`」。那个方向有两个没算到的成本：
  ①**没有 t2s 表** —— 仓库只 vendor 了 `opencc-s2twp.txt`（简→繁，53,607 行），
  反向要另写生成脚本再 vendor 一份数据。
  ②★**`title_chinese` 没有 SEO 闸门**。0022 专门造了 `title_hant_seo` 生成列，把
  `source='opencc'` 的机器转换值排除在 SEO 之外；而 `title_chinese` 既无 source 列
  也无对应 SEO 列，它**直接就是**页面标题、卡片标题和 JSON-LD 用的那个值。往里写机器
  转换结果等于绕过 0022 立的那条纪律，要补齐就得加 source 列 + 加 `title_chinese_seo`
  生成列 + 改所有 SEO 读取方 —— 那是个独立的大改动。
- **Context:** 剩下的 57 行（有简无繁、用户敲繁体）需要 t2s，仍然没有。只有 57 行，
  值不值得为它再 vendor 一份表可以以后再判。
- **Depends on / blocked by:** 建在本地搜索之上（迁移 0027 的 `title_hant` trgm 索引
  已经能被这个方案直接用上）。
