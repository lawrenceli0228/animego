...
**2026-09-03 更新：阈值已从分布定出，产出方已加守卫。** 按**比值**（写入的最高集号 ÷ 目录集数）重新分档，比按绝对溢出量清楚得多：

| 比值 | 番剧 | 越界行 | id-map 确认 | 平均目录集数 |
|---|---|---|---|---|
| ≤1.10x | 122 | 183 | 89% | 29.1 |
| 1.10–1.50x | 95 | 423 | 75% | 17.1 |
| 1.50–2.00x | 198 | 1,829 | 63% | 10.9 |
| **>2.00x** | **475** | **17,880** | 41% | **4.2** |

>2.00x 一档占全部损坏的 **88%**，平均目录集数 4.2（都是 ONA/SP/短片），最坏一部 3,528 个集位。≤1.10x 一档 89% 由独立映射确认、平均 29 集、总共才 183 行——那是目录少算一两集，**标题是对的，不能碰**。

★★**这不是绑错，是范围不符**：>2.00x 那 475 部里有 195 部绑定被 id-map 确认。Bangumi 合法地用一个 subject 覆盖 AniList 拆成正片+短片的内容，所以**解绑是错的修法**。

已做：`episodetitles.ScopeExceeded` 以 2.00x 为界，CLI 记 `SCOPE_MISMATCH`、sweep 记 rejected，**整条拒绝而不是截断**（列表属于整个系列时，它的第 1 集是系列的第 1 集不是本条目的，头和尾一样不可信）。目录集数未知的 2.9% 放行——没有分母就没有判据。**未做：存量那 20,315 行的清理**，以及 T9 本身。
...
**2026-09-04 更新** — 同一个函数上**另一半**已经修了：dandanplay 把应用层错误放在 **HTTP 200 的 body 里**（`{"success":false,"errorCode":429}`），而 `do()` 只看状态码、全仓从没解析过 `success`/`errorCode`，所以**配额耗尽和「这个条目没数据」完全同形**。现已加 `apiStatus` + `checkStatus`，返回 `ErrQuotaExceeded` / `ErrUpstreamRejected`。本条剩下的 4xx 部分**仍未做**：401/403/404 和其他 4xx 依然一律返回 `(nil, nil)`。两者是同一处症结的两面，但状态码那面还在。
...
**2026-09-04 收尾** — 已部署（migration 0031，prod `767326bf3522`）并排干完毕：**811 行全部离开 version 1，804 行标为上游不可读，7 行实际读到了并完成 V2 富化，0 行丢失绑定**。`bangumi_version = 1` 这个状态现在在目录里已经不存在，后台「Re-enrich v1」显示 0 并自动置灰。那 7 行（0.86%）正是 migration 拒绝在迁移里批量回填的理由——从 59 个样本推广会把它们一起标错。
...
## dandanplay 的额度是按功能组按天算的，而回填和线上弹幕共用同一个组

**What** — 给共享额度加一层显式预算与观测：回填运行前后记录各组用量，并让 `/match` 在番剧详情组耗尽时给出可区分的错误，而不是和「这个条目上游确实没有」返回同一个 404。

**Why** — 额度结构是：每个功能组**每天 5,000 次**、每月 120,000 次。`/api/v2/bangumi/bgmtv/:id` 和 `/api/v2/bangumi/:animeId` 都归在**番剧详情**组，而这个组同时是 `/match` 取分集列表用的组（Phase 1 和 Phase 2 都走它）。所以一次不设上限的回填不只是自己跑到停——**它会把线上弹幕功能一起带下线到当天结束**。这是实测的：2026-09-03 一次分集标题回填把该组跑到 5,000/5,000，之后线上 `/api/dandanplay/episodes?bgmId=291171` 对**同一轮跑批两小时前刚成功读过分集标题的 subject** 返回 `Anime not found on dandanplay`。

**Context** — 已经做了的一半：`--limit` 默认从 0（无上限）改成 4,500，留出一成给线上；同时修了缓存命中被计进请求数导致这个预算失准的问题。没做的一半是观测——现在要靠人去看控制台才知道用了多少，代码里没有任何一处知道自己离上限还有多远。另外**月额度完全不是瓶颈**（当月只用到 8%），唯一的稀缺资源是「天」，所以长回填的正确形态是跨天分批而不是一次跑完。

**Depends on / blocked by** — 无。与 `## dandanplay client 把所有 4xx 压成「没有数据」` 是同一处症结的两面：那一条让调用方分不清故障与稀疏，这一条说明分不清的代价有多大。

## 分集标题：从没被问过的存量已清零，剩下 4,138 行是绑定问题不是进度问题

**What** — 不再有「再跑一轮」这个动作。要么给「问过」单独记一列（见下），要么分别处理剩下三类的绑定质量问题。

**Why** — 2026-09-05 那轮把最后 2,709 行**从没被问过**的候选一次跑完（`SKIPPED` 0，在预算内收尾）。此后剩余的 4,138 个候选**全部至少被问过一次**，构成约为：`NO_TITLES` ~2,489（dandanplay 索引里没有这个 subject）、`UNDECIDED` ~1,021（没有任何独立来源为绑定背书）、`DDP_REBIND` ~575（dandanplay 说这个 subject 属于另一个 anilist 条目）、`SCOPE_MISMATCH` ~42。再跑一轮 = 花 4,138 次共享额度把上一轮的答案重听一遍，而这三类每一类都要靠改绑定、不是靠再问一次。

**Context** — 2026-09-05 实测。★★ **`--resume` 跳过的是「写成功」不是「问过了」**：`episode_titles_at` 只在写标题的事务里打戳，所以答不出来的行永远留在候选列表头部。今天第一次启动就撞上这个——200 行只写 1 行、196 次请求，照这个走法 4,000 次预算的 68% 会花在昨天已经判过的行上。★ 修法是新加的 `--start-after-id N`（PR #155），**预算工具不是正确性工具**：跳过的行仍是候选、仍没打戳。断点 `141932` 是反推的——重建 09-04 那轮的候选集（6,910 行），它最后一次写入排在第 4,132 位，正好是 4,000 次预算耗尽处。★ 那一轮成果：WRITTEN 1,296 / NO_TITLES 969 / UNDECIDED 265 / DDP_REBIND 159 / SCOPE_MISMATCH 20，2,628 次请求；库里 +1,040 条新标题、+2,327 条中文名、+68 部番剧首次有标题、打戳 8,092 → 9,390。★★ **报告里的 `titles written` 数的是被 upsert 碰到的行不是新增行**（`UpsertEpisodeTitleSourced` 的 `DO UPDATE` 没有 WHERE），所以 15,441 与库里 +1,040 并不矛盾——同一个口径问题对 3.26.0 记的 17,646 同样成立。

**Depends on / blocked by** — 无。

## 把「问过」和「写过」分开记，`--resume` 才名副其实

**What** — 给 `anime_cache` 加一列（比如 `episode_titles_asked_at`）在**每次真的走到网络之后**打戳，与只在写入事务里打的 `episode_titles_at` 并存；`--resume` 读前者，`--start-after-id` 就可以退休。

**Why** — 现在这两件事共用一个戳，而它只记录后者。后果是每一轮回填的头部都是上一轮答不出来的行：2026-09-05 那轮开跑时，5,436 个候选里 2,730 个属于这一类，占一天预算的 68%，而它们的判决（`NO_TITLES` / `UNDECIDED` / `DDP_REBIND`）不会隔夜改变。`--start-after-id` 用一个人工传入的 id 绕开了它，代价是这个 id 得靠反推、而且只在「上一轮是被预算截断的」这个前提下成立。

**Context** — 2026-09-05 加 `--start-after-id` 时记下。⚠️ 新列必须**只在真的发出了请求之后**打戳（`asked_upstream`），缓存命中不算——否则同一轮里共享 subject 的第二行会被误标为问过。另外要想清楚重问周期：`NO_TITLES` 有朝一日可能变成有（dandanplay 补录），所以这个戳应该是「多久之后可以再问一次」而不是「永不再问」，形状和 `ListReleasingEpisodeTitleCandidates` 的 `stale_after` 一样。

**Depends on / blocked by** — 无。

## dandanplay 交叉链接补 bgm_id：**探针已跑完，结论是做 sweep**

**What** — 实现搜索驱动的 sweep：每行一次 `SearchAnime`，对前 2 个候选取 `FetchEpisodesByDandanAnimeID`，用 `onlineDatabases` 的 AniList 交叉链接**等值**作精度闸门，subject 未被占用才写。（原 What 是「先跑探针决定形态」，探针已于 2026-09-04 跑完。）

**Why** — id-map sweep 已经把它能答的 791 行全绑完了，剩下约 4,463 行 map 完全没有条目，dandanplay 是唯一够得着的来源。两个方案**证据结构完全相同**（搜索只负责召回，`onlineDatabases` 的 AniList 交叉链接**等值**才是精度闸门），差别只在覆盖面和成本：机会捕获零请求但只碰得到用户恰好播放的那一小撮；sweep 覆盖全部但约 8,900 次请求 ≈ 两天的日额度。没有数据就在这两者间选，是在拿论证代替测量。

**Context** — 探针是只读的，报告里带**命中位置直方图**——那个数就是 sweep 的每行请求成本（若命中全在第 1 位，候选上限可以降到 1，每行两次请求）。★ 已查清的两件事：**`description_cn_eligible` 不用改**（它要求 `bgm_id_map` 同意，而这条路只在 map 沉默处动手，所以这类绑定天然拿不到中文简介资格——爆炸半径最大的公开字段已被现有闸门挡住）；**dandanplay 没有配额查询接口**（开放文档无此端点、`dev.dandanplay.com` 那页是 SPA 不渲染、swagger 404），所以额度只能靠人看控制台。★★ 搜索走的是**搜索组**（独立额度，长期闲置），只有详情腿和分集标题回填抢同一个组——排期时这两件事要错开。

**2026-09-04 探针结果（n=600，全目录每 7 行取 1，非头部样本）**
...
绑定率 6.8%（95% CI 4.8%–8.9%），全量 4,465 行外推 **约 305 行可绑**，成本约 5,983 次请求。

**结论：做。** 成本落在两个不同的额度组上——约 4,465 次搜索走**搜索组**（该组常年闲置，月用量 363/120,000），约 1,518 次详情走**番剧详情组**（一天配额的 30%）。也就是一天跑完、约 305 个新绑定，而且是一次性的，这批不会快速再生。相比之下机会捕获对这批**几乎必然为零**：75.8% 的行 dandanplay 索引里根本没有，用户也不会去播这些冷门条目。

★ **候选上限从 3 降到 2 零代价**：命中位置直方图是 `#1 → 76, #2 → 4, #3 → 0`，第 3 位一次都没命中过。降到 1 会丢 5% 的命中（4/80），不划算。

★★ **75.8% 的 NO_SEARCH_HIT 是这条路的天花板，和匹配算法无关** —— dandanplay 索引里没有这些条目，再好的匹配器也够不到四分之三。所以「补 bgm_id」这件事的上界是 24.2%，不是 100%；剩下那部分只能靠扩充 `bgm_id_map` 或别的来源。

⚠️ **第一次跑的探针结果（0 BINDABLE）是假的，别拿来引用。** 那次番剧详情组额度已耗尽，而当时的客户端把「200 + success:false」读成「这个条目没数据」，于是 103 次详情调用全是配额拒绝却被记成了 `NO_ANILIST_LINK`。坐实它的证据是 dandanplay 后台的月度计数只 +1。客户端已修（见下一条）。
...
## sweep 和用户请求共用同一个 AniList 额度，没有优先级

**What** — 让后台 sweep 从一个**低优先级**的额度里取令牌，或者至少在检测到用户侧请求排队时让路：给 `anilist.Client` 加一个 `Priority` 维度，或者给 sweep 一条独立的、总和仍在 30/min 之内的更慢通道。

**Why** — 额度是 30/min（`x-ratelimit-limit`，2026-09-08 实测），全站共享一个 token bucket。今天这不成问题，因为两条 sweep 都是串行发请求，任一时刻队列里最多一个后台请求，用户最坏多等一个 `minInterval`（2.1s）。**但这个结论完全依赖「串行」这个实现细节，而那个细节没有任何东西守着**——谁把批次改成并发拉取（一个很自然的「加速回填」想法），用户侧的排队就会从 2.1 秒变成整批的长度。

**Pros** — 后台工作再也不能挤占前台；也让「加速 sweep」这件事变成可以安全考虑的选项。

**Cons** — `golang.org/x/time/rate` 没有原生优先级，要么两个 limiter 手工分配额度（分错就是浪费或超额），要么在 `do()` 里加一层等待队列。而今天的实际收益是零，因为串行已经够用。
