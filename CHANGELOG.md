...
同一次 main 上的 sandbox 失败是另一回事——`/seasonal/spring/2026` 预热 404，是已知的 Turbopack「`[lang]` 下嵌套动态段整段不编译」。但预热守卫打印的判据是错的，它说「缺 `generate-params` + 编译时间接近零 = dev-server 缺陷；有 `generate-params` + 几百 ms = 真回归」。这次的日志两条都不成立：

```
GET /anime/0             404 in 1624ms  (无 generate-params，路由完全正常)
GET /seasonal/spring/2026 404 in  650ms  (无 generate-params，第一次并不接近零)
GET /seasonal/spring/2026 404 in   35ms  ← 从第二次起才是几十毫秒
```

真正能分开两者的是**这条路由有没有属于它的 `○ Compiling <段>` 行**：没有，且重试稳定在几十毫秒，是 dev-server 缺陷；有，然后才 404，是页面自己决定的 404。守卫文案照此重写，并把上面这三行反例写进去。
