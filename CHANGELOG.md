...
上一条刚上线，看了一眼 Re:ZERO 第三季的页：别名一行是一个泰文名和一个俄文名。AniList 的 synonyms 大半是各个市场的译名，中文名和日文名通常就是 hero 已经印着的标题（别名行本来就去掉它们），剩下能进这行的，对中文读者多半是噪音。先整行撤掉，`alternateName` 也退回三个标题——页面不显示的东西结构化数据不该声称。挑别名的逻辑（`visibleSynonyms`，按文字排序再截断）留着，缺的是「哪种文字该给哪种读者看」这条政策，定了再放回来。
