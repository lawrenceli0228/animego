...
把同一部番的旧卡手动合并进新卡，下次打开库时两张卡连同底下所有集数一起不见了。文件都还在、索引也在，只是界面上哪里都点不到。

**成环了。** 自动去重扫描（每次进库都跑）按 `anilistId` 分组、总是把新行合并进**最早**那一行，而且读的是全部 series，包括已经被合并隐藏的。读者刚做的「旧 → 新」（新卡 `mergedFrom: [旧]`）于是被它反向再做一遍（旧卡 `mergedFrom: [新]`）。`performMerge` 只问「源是不是已经在目标的列表里」，从没问「目标是不是已经在源的卡里」，照写不误。而 `useLibrary` 的隐藏规则是「出现在任何 `mergedFrom` 里就隐藏」，环上每个成员都被别的成员列着——整个环一起消失，没有一张卡留下来装它们。

两半修法，缺一不可：

- **不再成环。** `performMerge` 在写入前检查目标是否已在源的卡里（沿 `mergedFrom` 传递闭包，含自合并），是就当作「已经是一张卡」返回 null。卡片菜单、批量合并、系列页、去重扫描四个入口都走这一处。
- **已经成环的库自己修好。** 去重扫描开头先跑 `repairMergeCycles`：每轮切掉一条在环上、且**最后写入**的 `mergedFrom` 项（按 override 行的 `updatedAt`，同时间取列表里更靠后的，因为只追加），再重新判断，直到无环。不在环上的合并一条不动。在上面那个场景里，最后写入的正是扫描那一笔，切掉它就把卡还给读者选的那一行。修过的环会让观看进度对账重新跑一次。

隐藏规则抽成了 `resolveMergedIds.ts` 的 `mergedAwayIds`，`useLibrary` 和测试用的是同一份，不是各写一份。`resolveMergedIds.ts` 里「环不可能发生」的注释是错的，改掉了。

测试：新文件 `mergeCycles.test.ts` 12 条、`mergeOps.test.ts` 4 条，`dedupeSeries.test.ts` 加 3 条（读者的旧→新合并不被反转、磁盘上已有的环被修好且不再复发、三份副本其中一份已手动合并最后只剩一张卡装全部三个），sandbox e2e 加 1 条（IndexedDB 里种一个互相合并的环，打开 `/library` 应恰好一张卡、存储里环已断、合并本身还在）。拆掉守卫、拆掉修复、把「切最后写的」改成「切最早写的」三处变异分别打红 5 / 1 / 5 条；e2e 在撤掉修复的代码上得到 0 张卡，与读者看到的一致。
...
部署后（`3dc7ff4`）：`/version.json` 为 `3dc7ff4b7d97`，源站本机和经 CF 各测一轮，耗时取 go-api 日志的 `duration_ms`。

- **不存在的 id**：第一次 308ms（问过 AniList 后 404），之后 0ms（命中负缓存）。CF 对 `/api` 是 `DYNAMIC`，第二次确实到了源站。前端 `/anime/<id>`、`/en/anime/<id>` 都答 404，不是 500。
- **10 个并发请求打同一个新的不存在 id**：全部 404。源站只有两条 250–300ms，都不到一个令牌间隔（2.1s），说明它们在同一次上游调用里；其余 8 条到得晚，0ms 命中负缓存。上游只打了一次。
- **库里没有、AniList 有的续作**：353ms 现场拉取并入库，`detail_fetched_at` 和 `trailer_checked_at` 都已打上；之后 0ms，页面 200。
- **只是旧的行**：4 条里 3 条约 330ms 刷新入库，1 条碰上令牌忙，15ms 返回旧行。★ 刚部署那几分钟，同样的请求 3/3 都是忙：`warm_season` 开机就逐页排队拿令牌，把桶占满了，跑完即恢复。验这条路径别在刚部署完的时候验。
- **不完整的行**：打了一条 `trailer_checked_at` 为空的行，走的是排队的 `Detail`，AniList 回 404，记一条 Warn 后返回旧行（200）。★ 顺带看清了这类行为什么一直补不全：它们是 AniList 已经删掉的条目（四条老番 Specials），库里还留着，每次缓存过期后被访问就再问一次。量很小，改动前也这样，但目前没有任何机制清理「我们有、AniList 已经没有」的行。
- CI 对新版本线上重跑：Playwright（prod）8 passed，Lighthouse 通过。
...
白名单不是黑名单：拉丁到 Extended-A（ASCII、西欧重音字母、罗马字长音）、通用标点和符号、CJK 各区（跳过谚文兼容字母）、全角、emoji、CJK 扩展平面；一个码点不在里面整条不留。★ **判的是文字不是语种**——意大利文、德文这种纯拉丁字母的译名分不出来，留着；越南文靳靳靠声调符号（Latin Extended Additional 区）被挡在外面。
...
### 详情页把阶段 2 的数据画出来：下集倒计时、别名、标签、制作委员会、人气

阶段 3 的第三步。0036 / 0038 两批列进库一天多了，页面上还是一个字没露。现在：

**hero 多一条「第 N 集 · 9月20日 周日 22:16 · 3 天后」。** 一个客户端叶子，不是第七个 fact：它是 hero 里唯一会在读者看着的时候变的东西。两件服务端做不对的事都放到了客户端——**钟**（`/anime/*` 是 ISR 加 CF 边缘，副本可能是几小时前的，烤进 HTML 的「3 天后」会以新闻的口气说一句悄悄错了的话，所以服务端只出集数和绝对时间，相对时间靠 `useSyncExternalStore` 对着 `Date.now` 算，每分钟刷一次）和**时区**（服务端和水合期都按 Asia/Shanghai 画，两边一致、无需 suppress，之后换成浏览器自己的时区；首页周表同一个选择）。播完就整条消失而不是写「已播出」——这个值只有行那么新，一天前的行说「第 1179 集 5 小时前播了」是把陈旧值当新闻。没季度的预告首播也走这条（AniList 对 NOT_YET_RELEASED 的 nextAiringEpisode 是第 1 集）。
...
★ **验错的方式也记一笔**：8 月那份审计已经指出这条链断了，而我这次拿一部番的 API 响应看到 `startDate` 有值，就写成「已修好」。那部恰好是存量。**详情页渲染了某字段 + 一条样本返回了它，不等于写入链通了**；要看的是最近一次 upsert 之后的行。
...
**我先前对影响的判断少算了一档。** 「sweep 是串行的，所以用户最坏多等一个 `minInterval`」对**单个**请求成立，对**一次要发 N 个请求的端点**不成立 —— `/schedule` 会翻页，每一页都排在 sweep 的一个请求后面，于是每页实际 ~4.2s，而它的预算 `scheduleQueryTimeout` 是 20s（照着旧的 700ms 间隔定的）。五页就出界。

`anilistRatingsBatch` 2000 → 500。**这个常量约束的从来不是「回填多快跑完」，而是「一次 pass 能占住共享限速器多久」** —— 注释现在把这句写在那里，因为下一个想加速回填的人会直接盯着前者。窗口 130s → 33s（每小时 3.6% → 0.9%）。代价只有**一次性**回填的墙钟：约 10 小时变约 37 小时；回填完进入稳态每季只剩 458 行，这个数不再起作用。

没有提高 `scheduleQueryTimeout`。让用户等 30 秒不是修复，是把症状藏起来 —— 窗口之外这个端点冷启只要 4.7 秒。
...
AniList 的 GraphQL 里没有「评分人数」这个标量，有的是 `stats.scoreDistribution` 这个按十分档的直方图——各档人数相加才是打过分的人数。`popularity` 不是它：那数的是把作品加进列表的人，大部分没打分。

### ★ 但真正坏掉的不是缺一列，是那些数从来没被重读过

一行的评分只在第一次富化时被写入，此后没有任何东西会再去读它。四条写 `average_score` 的 AniList 路径全是请求驱动的，Bangumi 的分数和票数由 V2 的尾巴写下，而 V2 每行一辈子只跑一次。

2026-09-07 在生产库上量出来的形状没有第二种解释——2026 四个季度的 Bangumi 平均票数：

    夏季（当时正在播）      8      最高    257
    春季                  262      最高  8,855
    冬季                1,270      最高 18,720

正在播的那一季不可能比它前面那一季冷清。每一行都是在番剧公布时被读过一次然后冻在那里，这三个数唯一测量的是**各季的行碰巧是多久以前被富化的**。

所以这次加的不是一列，是**读取时间戳**——两个，AniList 和 Bangumi 各一个。写成时间戳而不是布尔，理由和迁移 0032 的 `trailer_checked_at` 是同一条：布尔只答得了「问过吗」，答不了「什么时候问的」和「哪些行该再问一遍」，而第三个才是承重的那个。有了戳，「谁到期了」是一句 WHERE，「问过了，答案确实是零」也不再和「从没问过」长得一样。

节奏：**当年及以后**的行每季度重读一次；**其余的行读一次就不再读**。后半句是有意的，也正是戳买来的东西——以后想给存量行排一个更慢的周期，改的是那句 WHERE，每行最后一次读的时间已经记在那里了。

### 两条 sweep 而不是一条

AniList 那条按 id 批量取，一次请求 50 行（`Page(media: id_in: [...])`），全目录约 370 次请求；Bangumi 那条一行一次 subject 请求，走 `/match` 和弹幕查询共用的那个 800ms 令牌桶，同样的目录是一万三千次。折成一条，一个卡住的 Bangumi 会把 AniList 的刷新一起扣住，而两边的批量上限也没法各自设——那正是「半分钟跑完」和「跑四分钟」的差别。

顺带补上了 TODOS 里预告片那条记着的能力：「按 id 批量补一个 AniList 字段」的查询常量和客户端方法现在存在了，下一个要给存量行补的字段可以直接用，不用再想一遍。它也够得到按季度取数够不着的那批——目录里 4,500 行没有季度。

### 三处会静默出错的地方，都钉在 SQL 里而不是调用方

`average_score` 走 COALESCE：这条 sweep 一次走完全表，AniList 返回空值时保留已存的分数，而请求驱动的那四条路径照旧无条件覆盖。同一个空值在两处含义不同——那四条各自回答的是刚被点开的一行，这条错一次就是整个目录一起错，而且没有第二个来源能把分数找回来。

`updated_at` 只在数字真的变了时才推进。它是 sitemap 报给 Google 的 lastmod，一次季度扫全表如果每行都盖一遍，等于什么都没变却重新发布了整份 sitemap。

`anilist_score_votes` 的 CHECK 拒绝没有观测记录的人数，也拒绝负数。得由列来拒绝而不是调用方：sweep 会吞掉单行的写入失败，一个调用方放过去、约束挡下来的值，在生产上唯一的症状是**那一行悄悄不再刷新**。
...
### 一个空的预告片字段有两种意思，缓存必须记得是哪一种

目录里新增预告片元数据（迁移 0032）：季度、年度、精选三个列表返回 `trailerId` / `trailerSite`，详情返回 `trailer` 对象。只接受 YouTube 的 11 位视频 id，且这条规则写在列的 CHECK 约束上而不只是调用方的校验里——存进来的是 id 和站点，不是任意的嵌入地址。

难的部分不是取值，是空值。AniList 返回的空预告片有两种完全不同的含义：查询里选了 `trailer` 而它是空的，那是「这部番确实没有预告片」；查询里压根没选这个字段，那只是「没问过」。把后者当成答案，一次搜索就会把已经存好的预告片抹掉。

有五条路径会把 AniList 的数据写进目录缓存，它们用的不是同一条查询。原先靠每个调用点自己记得补一行赋值来区分，而四个该补的地方只补了两个——正是这个仓库在分集标题的来源列上栽过的同一种坑：不变量只活在约定里，数据库的 CHECK 抓不到，新增第六个调用点时会静默漏掉。现在「这次查询选没选 trailer」是归一化函数的一个参数，类型定义在发出查询的那个包里，不回答就编译不过；另有一条测试直接断言三个查询常量的选择面，把类型的说法和 GraphQL 文档的实际内容钉在一起。

记录用的是时间戳而不是布尔标记。布尔只能回答「问过了吗」，答不了「什么时候问的」，更答不了「哪些行该重新问一遍」——而后者是真实需求：开播前还没发预告片的新番，和当时站点不支持的条目，都被存成了「问过，没有」，没有时间戳就再也找不出它们。

顺带把 CI 的带标签编译门从单个目录放宽到整个模块。原先它只盯 `test/integration/`，于是写在被测代码旁边的带标签测试两个 job 都编译不到——既不在不带标签的那次，也不在按目录限定的这次。这个仓库为同一个机制静默丢过整整两个月的测试覆盖。

新列上线时全库是空的，而列表接口不会自己去补——只有季度预热和详情页访问会写。所以部署顺序是先跑迁移 0032、再部署 API，然后打一次 `POST /api/admin/warm-all?startYear=1940`。参数不能省：那个端点的缺省起始年份是 2014，而目录里的条目最早到 1940。顺序也不能反：先预热会把整批行的 `cached_at` 推到当前时间而查询时间戳仍是空，那正是这次要堵的那个窗口。没有季度的条目（剧场版 / OVA / 特典）预热取不到，它们在详情页第一次被访问时补齐。

---

## [3.32.0] - 2026-09-06

### bangumi 侧从来不撤回它不再持有的行

一条 upsert 只说「这个来源现在有什么」，说不了「它以前有、现在没有了」。dandanplay 那条写入路径从写下来就补上了这半——upsert、撤回本次没覆盖到的本源行、删掉撤回之后空掉的行、盖戳，一个事务。**bangumi 这条只做 upsert。** 于是一份变短了的列表会把旧尾巴留在新行下面，在表里和已经确认过的标题长得一模一样；上一批那种行是靠手写 `DELETE` 清掉的，因为没有任何写入方够得到它们。

接上撤回是容易的那一半。要想清楚的是随它一起的守卫。

### ★★ 守卫是窗口，不是阈值

bangumi 的列表**经常是不完整而不是变短了的**。持有 bangumi 来源行的番里，有一批本来就比自己这一季的集数还少，另有几百部中间是有洞的——**稀疏是这份数据的正常形状**。所以「这次没提到它」不能等于「删掉它」，而查询本身带的那个 `cardinality(kept) > 0` 看不出区别：两种情况都非空。

第一直觉是比行数：新的比现存的少就不撤。这个方向错在两处。

一，**它会否掉这件事本身要修的东西**。一部 4 集的 ONA 绑到一个 3528 集的 subject 上，修好之后缩水 99%，而那个缩水**就是修复**。

二，**这在本仓库是个新习语**。翻了一遍现有的守卫——source 标签的等级、`bangumi_version` 的 `GREATEST`、`title_hant` 的 manual 保护、描述回填的空位限定——**比的全是等级或集合，没有一个比行数**。

所以问一行的问题不是「有多少」，而是「这一集有没有可能属于这个条目」：

| 这一行 | 判定 |
|---|---|
| 不在本次列表里，**且超出本季集数** | 撤回。网格只渲染 1..episodes，写入方也不会再产生它 |
| 不在本次列表里，但**落在本季之内** | 加回保护集，并计数上报 |
| 条目没有集数 | 一行都不撤 |

最后一行不是边角情况，它是 `episodes_bgm` 写入的**全部**人口（那个作业的候选条件就是 `ac.episodes IS NULL`，推导集数正是它的工作），所以那个写入方被有意留在只 upsert 的路径上。房子里的对应先例是 `MarkEpisodesBgmAttempted`：只有 `rejected` 会清掉已存的值，`undecided` / `empty` / `error` 一律不动——**缺席证据不是反面证据**。

上报出来的 `rowsShielded` 是值得盯的那个数：它说的是 bangumi 抓回来比我们已有的还少这件事多久发生一次，而这个数决定了要不要给它建一条人工复核队列。

### 撤回走自己的事务

它跟在 upsert 之后而不是把 upsert 包进来，所以两个调用方依赖的 best-effort 契约没变：失败只赔撤回不赔标题，行照常盖戳——不盖戳的行会回到每一批的队头，为了重试一次可选的清理再花掉两次 bangumi 请求。撤回和删除这一对倒是必须原子：只撤不删会留下两列皆空且没有来源的行，而以后任何一次撤回都够不到它们，因为找它们的 `WHERE` 匹配的正是它刚刚抹掉的那个来源。

### 落地当天几乎什么都不会做，这是有意的

拿生产数据抽了 40 部番做模拟，逐个真去 bgm.tv 抓列表、跑真实的归一化：**撤回 1 行，新增 335 行。** 现在全库没有任何一行落在自己这一季的集数之外——上一轮的清理清掉了，而 #156 堵住了产生新的那条路。**这是保险，不是修复**：下一次尾巴出现时，代价是一次扫描而不是一条手写 `DELETE`。

### ★★ 一条变异活了下来，因为那条语句根本没被执行过

六个变异，第一轮活了一个：把撤回拿到的保护集换成原始抓取集。原因不是断言弱——是 `withdraw == 0` 时函数提前返回，而当时**每条测试都只有一个判定**（要么只有尾巴、要么只有空洞），谁都走不到那条语句。补了「同一部番同时有尾巴和空洞」这一格才杀掉。

★ **一条语句需要两种判定同时出现才会被执行时，只有单一判定的测试集不构成覆盖。**

另有一个变异只被单元测试杀掉、集成测试毫无反应，那本身是个发现：**撤回集完全由窗口决定，跟这次抓回什么无关**（保护集已经被窗口截到本季之内，所以超出窗口的行永远不在里面）。这条独立性是特性——降级的抓取只能把撤回缩到零，扩不大它。原来的注释把抓取集说得比它实际承重的更重要，已经改掉。

---

### 记下这个集号用的是哪套编号

产品里每一个本地集号都诞生在同一个函数里，而那个函数**此前一个测试都没有**，并且把一个关键事实丢掉了：它读到的数字，是本季自己的 1..N，还是整个系列连续数下来的。

两者作为数值无法区分。字幕组写 `[03]`、`第03話`、` - 03 `，对第二季的第三集和整部的第三集产生的是同一个字符串，而它到底指哪个是那个组的习惯，不写在文件名里。站点已经为猜这件事付过账：**曾有两万多行分集标题落在自己这一季的集数之外**，全部出自「这个数字看起来像本季的，那就是本季的」。

`parseEpisodeMeta` 现在返回 `numberSpace`，并且默认承认自己不知道。只有两种东西算证据：

| 文件名 | 集号 | 编号空间 | 依据 |
|---|---|---|---|
| `[S02E03]` | 3 | `perSeason` | 这套写法在构造上就把季和集分开了 |
| `[總第67][第03話]` | 3 | `perSeason` | 文件名同时给了两套数，我们读的是另一套 |
| `[總第67]` | 67 | `absolute` | 我们读到的就是它声明的那个系列级数字 |
| `[12]` | 12 | `unknown` | 最主流的那种写法，什么也证明不了 |
| `第二季 [第87話]` | 87 | `unknown` | 季度标记说的是属于哪一季，不是从哪儿开始数 |
| `One Piece - 1091` | 1091 | `unknown` | 它**事实上**是绝对集号，但文件名从没这么说 |

最后一行就是纪律本身：**猜对了的猜测仍然是猜测。缺失一律读作 unknown，绝不默认成 perSeason**——这和 `episodeOffset` 那条「0 是答案，undefined 不是」是同一条规矩的另一半。

为此把原来内联在函数里的正则表数组提成了模块常量，每一条带上「它能证明什么」。顺序、正则、分辨率黑名单、去技术标签后的兜底全部未动，函数签名也没动，所以没有任何调用方需要跟着改。

**字段目前只携带不消费。** 会用到它的三个消费方——网格的偏移推断、弹幕匹配请求、观看进度的换算——今天各自从 `episodeOffset` 加 `totalEpisodes` 去推，换掉推断来源是另一件带回归的事。现在先让证据和数字待在一起，免得将来要从一个没人留下的文件名里重新推一遍。

★ 顺带给这个模块补了**它有史以来第一批测试**，21 条，其中 9 条是给既有行为织的回归网——重构一个没人测过的函数，正是那种会悄悄出错的重构。其中一条断言是我自己写错了而代码是对的：`parseSeason` 有意只在明显的季尾位置认罗马数字，所以 `Show II [03]` 返回 null。现在两个方向都钉住了，反例用的是 `FF VII Remake`。

---

### 把对应关系的基数写进类型里

本地集号和上游分集之间的对应关系，**两个方向都不是一对一**，而 `map[集号]单个值` 说不出这件事。

**一对多是沉默的那个。** 两条上游记录可以带同一个集号——一个播出版一个导演剪辑版，或者一个重新编号过的条目——而第一趟匹配命中之后就 `break`。第二条从此不可达，且没有任何地方记下它存在过。下游每一个消费方读到的都是第一条，而且**看起来都是对的**：弹幕轨道、分集标题、「已匹配 N 集」的计数，以及匹配错了之后用户会去用的那个手动选择器的预选项。

**多对一是另一个方向，而它通常是真的、不是坏的。** 一个文件夹里同一集有两个文件、分别按本季和按系列编号（1 和 29），那就**确实**是两个本地文件对一集上游。

去重是那种一看就对的修法，而它是错的：非库播放路径的选集条就是从这张映射里建的（没有条目的文件会被跳过），去掉第二条映射会让那个文件的 chip 直接消失——**拿「弹幕放错」换「文件不见了」，是更差的交易。**

所以改的不是匹配。`BuildEpisodeLinks` 保留胜出那一级的全部候选，`BuildEpisodeMap` 把它压平成 wire 和播放器一直在用的那个一集一条的形状，而**压平从一个控制流关键字变成了一个有名字、有测试的步骤**。三个级别仍然是优先级而不是合并：精确数字匹配和「在列表里排第几」不是两个平等的意见。

行为逐字未变，JSON 契约没动，前端零改动。**既有的十条测试一字未改并且仍然全过——这才是「行为没变」的证明**，而不是提交信息里的一句声称。

之所以要赶在逐集映射层存在**之前**做：那一层的全部意义就是承载单个整数装不下的关系，而让它流经一个会静默只留第一条的函数，等于在入口处又压扁一次。

### 另外

- 本地 `internal/dandanplay` 的容器化测试会间歇性报 `pull access denied for animego-postgres`——**注意少了 `:dev` 标签**，而 `testutil` 里传的明明是带标签的。重跑即绿。它会把 `go test ./...` 整个染红，排查时别当成自己的回归。
- `VERSION` 停在 3.22.0，与本文件的版本号早已不同步。本次未动。
