...
- [x] `docs/migration/ERD.md` 写完(独立文件):14 表 ASCII ERD + 索引概览 + extensions + ObjectId → UUID 映射规则。同步生成 `ERD.html`
- [x] `go-api/README.md` 大规模 rewrite:Quick Start 7 步 / 完整目录结构 / Schema overview 引用 ERD / 命令表扩 9 行(单测/集成/migrate-mongo/parity-check/truncate-retry)/ 工具链注释 cosmtrek → air-verse / **6 段 P1 lessons 学到的 gotchas**(Mongoose 命名 / mongo-driver bson.D / GOOS 后缀 / ConflictTarget per-table / FK ordering / bangumi_version / episodeTitles dedup)
- [x] `scripts/migrate-mongo-truncate-and-retry.sh`(72 行 bash):TRUNCATE 14 表 RESTART IDENTITY CASCADE → 跑 migrate-mongo --commit → 报每表 row count。env=dev/prod 切换 compose 文件 + DATABASE_URL。Cutover rehearsal 用,P9 cutover 不用(prod 是首次干净写入)
- [x] `docs/migration/MIGRATION_PLAN.md` § 7 状态表:P0 + P1 flip 到 **DONE**,链到 P0-PROGRESS.html + P1-PROGRESS.html + ERD.html
- [x] HTML 重生:`P1-PROGRESS.html` / `ERD.html` / `MIGRATION_PLAN.html`

---

## P1 → P2 退出条件(critical gate)✓

- [x] 7 transforms 全 testify 单测,**91.2% 覆盖**(目标 80%)
- [x] testcontainers 集成测试全过,**7/7 PASS 9.76s wall**(含 pg_cron TTL test)
- [x] dry-run on prod mongodump:**6493 → 95008 rows 100% transformed, 0 failures**,parity 10×1000 = **100.00% match**
- [x] pg_cron TTL 真触发删过 18-month 老 row(integration test 直接 exec TTL SQL,等价 cron 调度)
- [x] ERD diagram 落进 [`ERD.md`](ERD.md) + 链到 `go-api/README.md`

**P1 → P2 gate 全过,可开 P2.1(/api/anime/* + 富化 queue)**。
