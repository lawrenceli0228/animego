...
export default function ContinueWatching() {
  ...
  return (
    <section style={{ marginTop: 40 }}>
      ...
      <div style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 8, scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.3) transparent' }}>
        {list.map(item => (
          ...
          >
            <div style={{ position: 'relative' }}>
              ...
              {item.episodes > 0 && (
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: 'rgba(0,0,0,0.5)' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, (item.currentEpisode / item.episodes) * 100)}%`, background: 'linear-gradient(90deg,#7c3aed,#06b6d4)' }} />
                </div>
              )}
              <div style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(10,14,26,0.75)', backdropFilter: 'blur(4px)', borderRadius: 6, padding: '2px 6px', fontSize: 11, fontWeight: 600, color: '#a78bfa' }}>
            ...
            <div style={{ padding: '8px 10px 10px' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#f1f5f9', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', lineHeight: 1.4 }}>
                {pickTitle(item, lang)}
              </div>
            </div>
          </Link>
