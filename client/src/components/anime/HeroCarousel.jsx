          ...
          <div key={anime.anilistId} style={{
            ...
            <div className="container" style={{ position: 'relative', height: '100%', display: 'flex', alignItems: 'center' }}>
              <div style={{ maxWidth: 560, animation: i === current ? 'fadeUp 0.6s ease' : 'none' }}>
                ...
                <h1 style={{ fontSize: 'clamp(24px,3.5vw,46px)', fontFamily: "'Sora',sans-serif", fontWeight: 800, lineHeight: 1.15, marginBottom: 16, color: '#f1f5f9', textShadow: '0 2px 20px rgba(0,0,0,0.6)' }}>
                  {pickTitle(anime, lang)}
                </h1>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
                  {(anime.genres || []).slice(0, 4).map(g => (
                    <span key={g} style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: 'rgba(124,58,237,0.2)', border: '1px solid rgba(124,58,237,0.35)', color: '#c4b5fd' }}>{g}</span>
                  ))}
                </div>
                {anime.description && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <Link to={`/anime/${anime.anilistId}`}
                    style={{ padding: '10px 28px', borderRadius: 10, background: 'linear-gradient(135deg,#7c3aed,#06b6d4)', color: '#fff', fontWeight: 700, fontSize: 14, fontFamily: "'Sora',sans-serif", textDecoration: 'none', boxShadow: '0 4px 20px rgba(124,58,237,0.35)', transition: 'opacity 0.2s' }}
