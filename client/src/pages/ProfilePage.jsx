...
export default function ProfilePage() {
  ...
  // Map subscriptions to anime shape for AnimeGrid
    ...
    <div className="container" style={{ paddingTop:40, paddingBottom:60 }}>
      ...
      <div style={{ marginBottom:36, animation:'fadeUp 0.4s ease' }}>
        <p style={{ color:'#7c3aed', fontSize:13, fontWeight:600, letterSpacing:'2px',
          textTransform:'uppercase', marginBottom:8 }}>我的列表</p>
        ...
        {STATUS_OPTIONS.map(opt => (
          <button key={opt.value} onClick={() => setActiveStatus(opt.value)}
            style={{
              ...
              background: activeStatus === opt.value
                ? `linear-gradient(135deg,${opt.color}33,${opt.color}22)`
                : 'transparent',
              color: activeStatus === opt.value ? opt.color : '#64748b',
              boxShadow: activeStatus === opt.value
                ? `0 2px 12px ${opt.color}30` : 'none',
              borderBottom: activeStatus === opt.value
                ? `2px solid ${opt.color}` : '2px solid transparent'
            }}>
            {opt.label}
          </button>
        ))}
      ...
      {animeList?.length === 0 && !isLoading ? (
        <div style={{ textAlign:'center', padding:'60px 0', color:'#64748b',
          ...
          还没有「{STATUS_OPTIONS.find(o=>o.value===activeStatus)?.label}」的番剧
        </div>
      ) : (
