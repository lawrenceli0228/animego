...
- [ ] `anilist.service.js` — 新增 `warmSeasonCache(season, year)` 全量预热函数
- [ ] `anilist.service.js` — 修复 `getSeasonalAnime` 翻页逻辑，改为全走 MongoDB
- [ ] `server/index.js` — 服务器启动后触发当前季度预热（后台异步，不阻塞）
- [ ] 验证：翻到第 2、3、N 页均有数据
- [ ] 验证：连续翻页不触发 AniList 请求（查看 server 日志）
