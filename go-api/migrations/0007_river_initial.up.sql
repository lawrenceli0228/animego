-- 0007_river_initial.up.sql
--
-- river (https://riverqueue.com) is the Postgres-backed job queue that
-- replaces the Express in-memory enrichment queues (bangumi.service.js).
-- This file is the verbatim concatenation of river v0.37.1 internal
-- migrations 001..004 from riverdriver/riverpgxv5/migration/main/, with
-- the `/* TEMPLATE: schema */` placeholders stripped.
--
-- Split rationale:  migration 004 ADDS a new value 'pending' to the
-- river_job_state ENUM, and migrations 005/006 USE that value in function
-- bodies.  Postgres forbids adding-then-using an ENUM value in the same
-- transaction.  golang-migrate wraps each migration file in one
-- transaction, so we split at the ENUM boundary: 0007 ends with the
-- ADD VALUE statement, 0008 contains the SELECT CASE/function body that
-- references the new value.
--
-- When river releases new internal migrations (007+), copy them in
-- chronological order into NEW go-api/migrations/00NN_river_*.up.sql
-- files.  Do NOT edit this file in place — applied migrations are
-- immutable per golang-migrate convention.
