...
func TestNormalizeMainRow_AllFields(t *testing.T) {
	...
	row := NormalizeMainRow(m, anilist.SearchDocument)
...
func TestNormalizeMainRow_NilTitleAndCoverImage(t *testing.T) {
	row := NormalizeMainRow(anilist.Media{ID: 42}, anilist.SearchDocument)
...
func TestNormalizeMainRow_BangumiColumnsNotSet(t *testing.T) {
	...
	var p any = NormalizeMainRow(anilist.Media{ID: 1}, anilist.SearchDocument)
	_ = p
}
