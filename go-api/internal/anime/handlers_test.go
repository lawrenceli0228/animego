...
// -----------------------------------------------------------------------------
// Seasonal — defaults, validation, pagination math, envelope shape.
// -----------------------------------------------------------------------------
...
func TestSeasonal_Defaults(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal", nil))
...
func TestSeasonal_InvalidSeason(t *testing.T) {
	t.Parallel()

	q := &fakeQuerier{}
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal?season=BOGUS", nil))

	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Contains(t, rec.Body.String(), `"VALIDATION_ERROR"`)
}

func TestSeasonal_PerPageCapped(t *testing.T) {
	t.Parallel()
	...
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal?perPage=500", nil))

	require.Equal(t, http.StatusOK, rec.Code)
	assert.Equal(t, int32(200), gotLimit, "perPage should be capped at 200")
}
...
func TestSeasonal_PageOffsetMath(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal?page=3&perPage=15", nil))
...
func TestSeasonal_EnvelopeShape(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal?season=SUMMER&year=2024", nil))

	require.Equal(t, http.StatusOK, rec.Code)
	body := rec.Body.String()

	// Parse the JSON and assert structurally — also verify the byte
	// shape contains "data" before "pagination" (encoding/json emits
	...
	// Verify data comes before pagination in the raw bytes.
	...
	// Verify pagination sub-fields appear in declaration order.
	...
	assert.True(t, pageIdx < perPageIdx, "page < perPage in order")
	assert.True(t, perPageIdx < totalIdx, "perPage < total in order")
	assert.True(t, totalIdx < totalPagesIdx, "total < totalPages in order")
}

func TestSeasonal_TotalPagesRoundUp(t *testing.T) {
	t.Parallel()
	...
	rec := httptest.NewRecorder()
	Seasonal(q).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/anime/seasonal?perPage=10", nil))
	...
	assert.Equal(t, 3, parsed.Pagination.TotalPages, "21 items / 10 per page rounds up to 3")
}

func TestSeasonal_DBError(t *testing.T) {
	t.Parallel()
