...
func TestResolveTitleLadder(t *testing.T) {
	...
	t.Run("no tier reaches the row", func(t *testing.T) {
		d := r.resolveTitle(Row{AnilistID: 777777})
		if d.Source != "" {
	...
	t.Run("anilist synonym fallback carries into the Decision", func(t *testing.T) {
		d := r.resolveTitle(Row{AnilistID: 21, TitleChinese: ptr("海贼王")})
...
func TestSourceHashCoversTheInput(t *testing.T) {
	...
	d := r.resolveTitle(Row{AnilistID: 1, TitleChinese: ptr("星际牛仔")})
	if d.Input != "星際牛仔" || d.Hash != hashOf("星際牛仔") {
		t.Fatalf("anilist tier hashed %q; want the dataset value", d.Input)
	}

	d = r.resolveTitle(Row{AnilistID: 16498, TitleNative: ptr("進撃の巨人")})
...
func TestResolveDescription(t *testing.T) {
	...
	d := r.resolveDescription(Row{DescriptionCN: ptr("人类得以在太阳系范围内自由移动。")})
	if d.Source != SrcOpenCC {
	...
	if got := r.resolveDescription(Row{}); got.Source != "" {
		t.Fatalf("a row with no description_cn proposed %q", got.Source)
	}
	if got := r.resolveDescription(Row{DescriptionCN: ptr("")}); got.Source != "" {
...
func TestCheckStaleFollowsTheClaimedSource(t *testing.T) {
	...
	if got := r.checkStale(Row{}, nil, ptr(SrcAnilist), nil); got != StaleNone {
...
func TestRestaleNarrowsTheWriteSet(t *testing.T) {
	...
	unfilled := Row{AnilistID: 1, TitleChinese: ptr("星际牛仔")}

	results := ClassifyAll(r, []Row{stale, unfilled})
...
func TestColumnsKeepsArraysAligned(t *testing.T) {
	rows := []RowResult{
		{Row: Row{AnilistID: 1}, Title: Decision{Source: SrcAnilist, Value: "星際牛仔", Hash: "h1"}},
		{Row: Row{AnilistID: 2}, Title: Decision{Source: SrcOpenCC, Value: "鬼滅之刃", Hash: "h2"}},
	}
