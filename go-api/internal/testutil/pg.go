...
var allTables = []string{
	...
	// Activity rollups (migration 0025).  user_activity_daily would be
	// reached by the CASCADE from users anyway; activity_surface_daily has no
	// foreign keys at all and would otherwise leak counts between tests, so
	// both are listed to keep this the readable inventory it claims to be.
	"user_activity_daily",
	"activity_surface_daily",
	"danmakus",
	"episode_windows",
