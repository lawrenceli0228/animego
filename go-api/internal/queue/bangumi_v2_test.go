...
// subjectFn / episodesFn to control the upstream behaviour.  Calls
...
// TestBangumiV2_HappyPath_FullUpdate — subject → 1 UpdateBangumiV2 call
// with score, votes and Chinese title, all args correct.
...
// TestBangumiV2_ParallelFetch_BothCalled — under the happy path the two
// Bangumi endpoints V2 still reads get hit exactly once each.  The
// characters endpoint is deliberately absent from this list: see the
// package doc for why V2 stopped asking for it.
func TestBangumiV2_ParallelFetch_BothCalled(t *testing.T) {
	...
	assert.Equal(t, 1, b.episodesCalls, "Episodes called exactly once")
