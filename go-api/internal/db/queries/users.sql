...
SET previous_refresh_token = refresh_token,
    refresh_token          = sqlc.arg(new_token),
    refresh_rotated_at     = now(),
    updated_at             = now()
WHERE id = sqlc.arg(id)
  AND refresh_token = sqlc.arg(expected_token)
RETURNING username, role;

-- name: ClearRefreshTokenIfMatches :execrows
...
SET refresh_token          = NULL,
    ...
    updated_at             = now()
WHERE id = sqlc.arg(id)
  AND (refresh_token = sqlc.arg(presented_token)
       OR previous_refresh_token = sqlc.arg(presented_token));
