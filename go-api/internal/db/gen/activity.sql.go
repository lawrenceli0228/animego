...
const getActivityRetention = `-- name: GetActivityRetention :one
WITH bounds AS (
    SELECT
        (now() AT TIME ZONE 'Asia/Shanghai')::date                                   AS today,
        (now() AT TIME ZONE 'Asia/Shanghai')::date
            - ($1::integer - 1)                                 AS earliest
),
cohort AS (
    SELECT
        u.id                                                AS user_id,
...
type GetActivityRetentionRow struct {
	D1Cohort     int64 `json:"d1Cohort"`
	D1Returned   int64 `json:"d1Returned"`
	D7Cohort     int64 `json:"d7Cohort"`
	D7Returned   int64 `json:"d7Returned"`
	EverCohort   int64 `json:"everCohort"`
	EverReturned int64 `json:"everReturned"`
}
...
func (q *Queries) GetActivityRetention(ctx context.Context, windowDays int32) (GetActivityRetentionRow, error) {
	row := q.db.QueryRow(ctx, getActivityRetention, windowDays)
	var i GetActivityRetentionRow
	err := row.Scan(
		&i.D1Cohort,
		&i.D1Returned,
		&i.D7Cohort,
...
type GetActivitySnapshotRow struct {
	Dau               int64       `json:"dau"`
	Wau               int64       `json:"wau"`
	Mau               int64       `json:"mau"`
	InstrumentedSince pgtype.Date `json:"instrumentedSince"`
}
...
// See 0026's header.
func (q *Queries) GetActivitySnapshot(ctx context.Context) (GetActivitySnapshotRow, error) {
	row := q.db.QueryRow(ctx, getActivitySnapshot)
	var i GetActivitySnapshotRow
	err := row.Scan(
		&i.Dau,
		&i.Wau,
		&i.Mau,
...
const getAdminUserActivityCounts = `-- name: GetAdminUserActivityCounts :many
SELECT
    u.id                                          AS user_id,
...
func (q *Queries) GetAdminUserActivityCounts(ctx context.Context, dollar_1 []uuid.UUID) ([]GetAdminUserActivityCountsRow, error) {
	rows, err := q.db.Query(ctx, getAdminUserActivityCounts, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetAdminUserActivityCountsRow{}
	for rows.Next() {
...
const listActivityDailyTotals = `-- name: ListActivityDailyTotals :many
SELECT
    activity_date,
...
type ListActivityDailyTotalsRow struct {
...
func (q *Queries) ListActivityDailyTotals(ctx context.Context, dayCount int32) ([]ListActivityDailyTotalsRow, error) {
	rows, err := q.db.Query(ctx, listActivityDailyTotals, dayCount)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListActivityDailyTotalsRow{}
	for rows.Next() {
		var i ListActivityDailyTotalsRow
		if err := rows.Scan(
			&i.ActivityDate,
			&i.ActiveUsers,
			&i.Requests,
...
const listActivitySurfaceTotals = `-- name: ListActivitySurfaceTotals :many
SELECT
    surface,
...
type ListActivitySurfaceTotalsRow struct {
	Surface     string `json:"surface"`
	AuthedCount int64  `json:"authedCount"`
...
// table.
func (q *Queries) ListActivitySurfaceTotals(ctx context.Context, dayCount int32) ([]ListActivitySurfaceTotalsRow, error) {
	rows, err := q.db.Query(ctx, listActivitySurfaceTotals, dayCount)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListActivitySurfaceTotalsRow{}
	for rows.Next() {
		var i ListActivitySurfaceTotalsRow
		if err := rows.Scan(
			&i.Surface,
			&i.AuthedCount,
			&i.AnonCount,
...
const listNewUserCountsByDay = `-- name: ListNewUserCountsByDay :many
SELECT
    (created_at AT TIME ZONE 'Asia/Shanghai')::date AS signup_date,
    count(*)::bigint                                AS new_users
FROM users
WHERE (created_at AT TIME ZONE 'Asia/Shanghai')::date
      >= (now() AT TIME ZONE 'Asia/Shanghai')::date - ($1::integer - 1)
GROUP BY 1
ORDER BY 1
`

type ListNewUserCountsByDayRow struct {
	SignupDate pgtype.Date `json:"signupDate"`
...
func (q *Queries) ListNewUserCountsByDay(ctx context.Context, dayCount int32) ([]ListNewUserCountsByDayRow, error) {
	rows, err := q.db.Query(ctx, listNewUserCountsByDay, dayCount)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListNewUserCountsByDayRow{}
	for rows.Next() {
		var i ListNewUserCountsByDayRow
		if err := rows.Scan(&i.SignupDate, &i.NewUsers); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
