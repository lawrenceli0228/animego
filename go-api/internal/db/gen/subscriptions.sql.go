...
func (q *Queries) GetSubscription(ctx context.Context, userID uuid.UUID, anilistID int32) (GetSubscriptionRow, error) {
...
func (q *Queries) ListWatchedEpisodes(ctx context.Context, userID uuid.UUID, anilistID int32) ([]int32, error) {
	rows, err := q.db.Query(ctx, listWatchedEpisodes, userID, anilistID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
WITH target AS (
    ...
    SELECT t.user_id, t.anilist_id, $3::integer
    FROM target t
    ...
    SELECT ew.episode
    FROM episode_watches ew
    WHERE ew.user_id = $1::uuid
      AND ew.anilist_id = $2::integer
    UNION
...
type MarkEpisodeWatchedRow struct {
	WatchedEpisodes []int32 `json:"watchedEpisodes"`
	CurrentEpisode  int32   `json:"currentEpisode"`
}
...
func (q *Queries) MarkEpisodeWatched(ctx context.Context, userID uuid.UUID, anilistID int32, episode int32) (MarkEpisodeWatchedRow, error) {
	row := q.db.QueryRow(ctx, markEpisodeWatched, userID, anilistID, episode)
	var i MarkEpisodeWatchedRow
	err := row.Scan(&i.WatchedEpisodes, &i.CurrentEpisode)
	return i, err
}

const markEpisodesWatched = `-- name: MarkEpisodesWatched :one
WITH target AS (
    ...
    FROM target t
    CROSS JOIN unnest($3::integer[]) AS e(episode)
    ON CONFLICT (user_id, anilist_id, episode) DO NOTHING
    RETURNING episode
), watched AS (
...
type MarkEpisodesWatchedRow struct {
	WatchedEpisodes []int32 `json:"watchedEpisodes"`
	CurrentEpisode  int32   `json:"currentEpisode"`
}
...
func (q *Queries) MarkEpisodesWatched(ctx context.Context, userID uuid.UUID, anilistID int32, episodes []int32) (MarkEpisodesWatchedRow, error) {
	row := q.db.QueryRow(ctx, markEpisodesWatched, userID, anilistID, episodes)
	var i MarkEpisodesWatchedRow
	err := row.Scan(&i.WatchedEpisodes, &i.CurrentEpisode)
	return i, err
}

const unmarkEpisodeWatched = `-- name: UnmarkEpisodeWatched :one
WITH target AS (
    SELECT s.user_id, s.anilist_id
    FROM subscriptions s
    WHERE s.user_id = $1::uuid
...
type UnmarkEpisodeWatchedRow struct {
	WatchedEpisodes []int32 `json:"watchedEpisodes"`
	CurrentEpisode  int32   `json:"currentEpisode"`
}
...
func (q *Queries) UnmarkEpisodeWatched(ctx context.Context, userID uuid.UUID, anilistID int32, episode int32) (UnmarkEpisodeWatchedRow, error) {
	row := q.db.QueryRow(ctx, unmarkEpisodeWatched, userID, anilistID, episode)
	var i UnmarkEpisodeWatchedRow
	err := row.Scan(&i.WatchedEpisodes, &i.CurrentEpisode)
	return i, err
}
...
WITH previous AS (
    ...
    INSERT INTO episode_watches (user_id, anilist_id, episode)
    SELECT p.user_id, p.anilist_id, $3::integer
    FROM previous p
    WHERE $3::integer BETWEEN 1 AND 5000
    ON CONFLICT (user_id, anilist_id, episode) DO NOTHING
    RETURNING episode
), watched AS (
