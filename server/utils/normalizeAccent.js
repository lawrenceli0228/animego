/**
 * Normalize an AniList `coverImage.color` hex into a reliable UI accent.
 *
 * AniList returns a k-means dominant hue per cover. ~30% of titles come back
 * as washed pastels or near-neutrals that don't register as accent when applied
 * at low opacity. This util clamps OKLCH chroma and lightness so every accent
 * carries visible weight, and precomputes the RGB triplet + WCAG contrast on
 * black so clients can consume a finished token without runtime color math.
 *
 * Returns { raw, accent, accentRgb, accentContrastOnBlack }.
 * Falls back to brand violet (#8B5CF6) for null/invalid/grayscale input.
 */
...
const GRAYSCALE_THRESHOLD = 0.005; // raw chroma below this = no meaningful hue
...
function normalizePosterAccent(input) {
  ...
  return {
    ...
    accentContrastOnBlack: contrastOnBlack(result),
  };
}

module.exports = { normalizePosterAccent };
