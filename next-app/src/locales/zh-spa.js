const zh = {
  ...
  torrent: {
    ...
    titleHintTitle: '搜不到？换个标题再试',
    titleHintBody: '不同字幕组用的名字不一样。点下面的标题按钮，会按那个名字重新搜索，命中率通常更高。',
  },
  playerGuide: {
    eyebrow: '本地弹幕播放器',
  ...
  communityDiscovery: {
    ...
    libraryGuideEyebrow: '我的库 · 本地播放',
    libraryGuideTitle: '下载好的番剧，在我的库点开就能播',
    libraryGuideBody: '本地文件自动归到对应的番剧，带弹幕直接播放，追番进度一起记下来。',
    libraryGuideStepDownload: '用任意工具把番剧下载到本地。',
    libraryGuideStepFolder: '首次打开我的库，授权下载目录。',
    libraryGuideStepRefresh: '之后新剧集自动入库，点开即播、弹幕自动匹配。',
  ...
  library: {
    ...
    drop: {
      title: '把文件夹拖到这里',
      sub1: '自动识别番剧、归类成系列、对上集号。',
      sub2: '视频在你电脑上,不上传。',
      cta: '选择文件夹',
      ariaIdle: '把文件夹拖到这里或选择文件夹',
      ariaHover: '准备接收文件夹',
      ariaParsing: '导入解析中',
      hoverTitle: '放下即可开始',
      hoverDetected: '检测到 ',
      ...
      parsingTitle: '解析中…',
      parsingCancel: '⊘ 取消',
      parsingProgressLabel: '文件解析进度',
      parsingEtaPrefix: 'ETA',
      parsingEtaUnit: '秒',
      releaseToImport: '// 释放以导入',
    },
    filter: {
      recent: '最近播放',
      new: '新加入',
    ...
    bulk: {
      ...
      selectAll: '全选',
      delete: '删除',
      mergeIntoSeries: '合并为系列',
    },
    rhythm: {
      cadenceLabel: '// CADENCE',
      epsThisWeek: '集本周',
      daysStreak: '天连续',
      cal14Aria: '近 14 天: {{days}} 天有记录',
    },
    row: {
    ...
    watch: {
      noObserverHint:
        '此浏览器不支持文件夹实时监听,页面打开期间每 2 分钟自动检查一次新文件。',
    },
    persistHint:
    ...
    importDrawer: {
      ...
      title: '导入本地文件',
      close: '关闭',
      clusterCount: '共 {{total}} 个集群 · 进度 {{pct}}%',
      parseProgress: '// 解析进度',
      statMatched: '成功',
      statRunning: '解析中',
      statAmbiguous: '待选',
    ...
    opsLog: {
      ...
      timeHoursAgo: '{{n}} 小时前',
      timeDaysAgo: '{{n}} 天前',
      summaryMergeBoth: '合并 {{source}} → {{target}}',
      summaryMergeTarget: '合并到 {{target}}',
      summaryMerge: '合并',
      summarySplitName: '拆分出新系列「{{name}}」',
      summarySplit: '拆分系列',
      summaryRematchTarget: '重新匹配为 {{target}}',
      summaryRematch: '重新匹配',
      summaryUnfile: '从系列移除',
      summaryDelete: '删除',
    },
    ...
    confirm: {
      deleteSingle: '从库里删除「{{title}}」?\n\n会清掉本地的元数据 / 进度 / 覆盖,但磁盘上的视频文件不会被动。',
      deleteBulkPreviewMore: '… (共 {{total}} 项)',
      deleteBulk: '从库里删除以下 {{count}} 项?\n\n{{preview}}\n\n会清掉本地的元数据 / 进度 / 覆盖,但磁盘上的视频文件不会被动。',
      resetLibrary: '清空整个本地库?将删除全部 series / episodes / fileRefs / matchCache / fileHandles / opsLog。\n(磁盘上的视频文件不会被动)',
    },
    toast: {
      deleteSuccess: '已删除「{{title}}」',
      deleteSuccessCount: '已删除 {{count}} 项',
      deleteSuccessPartial: '已删除 {{ok}}/{{total}} 项,部分失败',
      deleteFailed: '删除失败,请重试',
      mergeFailed: '合并失败,请重试',
      dedupeNone: '库里没有重复的系列',
      dedupeSkipped: '检测到 {{groups}} 组重复但都已合并过 (skip {{skipped}})',
      dedupeSuccess: '合并 {{merged}} 项 · 共 {{groups}} 组重复',
      danmakuUpdated: '弹幕来源已更新',
      danmakuUpdateFailed: '更新失败',
    },
    undoToast: {
      mergeFromSource: '从 {{source}} 合并',
      mergeBulkTitle: '合并到 {{target}}',
      mergeBulkMeta: '{{count}} 项',
      dedupeBulkTitle: '合并重复 ({{count}} 项)',
      dedupeBulkMeta: '{{groups}} 组 · 同 animeId',
      autoMergeMeta: '来自 {{folderCount}} 个文件夹 ({{folderLabels}})',
      kicker: '已合并',
      undoLabel: '撤销',
      viewLabel: '查看',
    },
    ...
    unavailable: {
      count: '{{count}} 项暂时不可访问',
      reauthTitle: '对每个离线文件夹弹出浏览器原生授权框,逐个重新授权',
      reauthorizing: '授权中…',
      reauthorize: '重新授权',
      refreshTitle: '重新探测硬盘连接状态(不会弹授权框)',
      refreshing: '检测中…',
      refresh: '刷新可用性',
      reasonPartial: '部分集缺失',
      reasonOffline: '硬盘未连接',
      openBtn: '打开',
      deleteBtn: '删除',
      deleteTitle: '从库里删除（不会动磁盘文件，重连后可重新导入）',
    },
    // SeriesDetailSheet
    detail: {
      close: '关闭',
  ...
  admin: {
    ...
    hant: {
      ...
      // 唯一一种让这个数字看起来无害的读法。
      lede: '富化只写简体列，繁体列要等回填才动。落后的行不会留空——繁体读者读到的是简体正文。',
      titlesRow: '标题',
      descsRow: '简介',
      titleBehind: '标题落后',
      descBehind: '简介落后',
      // 提示语就是 SQL 谓词的白话版，好让人能自己复核这个数。
      titleBehindHint: '有简体标题，没有繁体',
      descBehindHint: '有简体简介，没有繁体',
      // 零需要一句话说出来。两个没人解释的灰色 0 一样容易被读成
      // 「这块坏了」，那是同一个误读的反方向。
      inSync: '没有落后的行——每条简体都有对应的繁体。',
      serpEligible: '人工来源标题',
      machineConverted: '机器转换',
      // 这两个数之间的差是整块面板上唯一一个「看着像 bug 其实不是」的地方，
      // 所以写死在屏幕上，不靠人去问。
      serpHint: '只有这些能进 <title> 和 JSON-LD；其余是机器转换的，不进搜索结果。',
      lastRun: '上次回填',
      runningNow: '回填进行中',
      runNow: '立即回填',
      enqueued: '已排入回填队列。',
      loadError: '繁体漂移数据读取失败',
    },
