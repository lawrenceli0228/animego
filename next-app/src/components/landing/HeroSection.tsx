 ...
 *
 * NOTE: comments in this file are ASCII-only. Turbopack code-frame highlighter
...
const s = {
  ...
  bgTrail: (top: number, delay: number, duration: number, width: string): CSSProperties => ({
    position: "absolute",
    top: `${top}%`,
    left: "-10%",
    height: 1,
  ...
  h1Period: (lang: Lang): CSSProperties => ({
    display: "inline-block",
  ...
  btnGhost: {
    ...
    transition:
      "border-color 150ms var(--ease-out-expo), color 150ms var(--ease-out-expo), background 150ms var(--ease-out-expo)",
    textTransform: "uppercase",
  } as CSSProperties,
  metaRow: {
    marginTop: 40,
  ...
  showcase: (hue: number | string): CSSProperties => ({
    position: "absolute",
    inset: 0,
    borderRadius: 12,
    overflow: "hidden",
  ...
  posterTint: (hue: number | string): CSSProperties => ({
    position: "absolute",
    inset: 0,
  ...
  showcaseTopDot: (hue: number | string): CSSProperties => ({
    width: 6,
    height: 6,
    borderRadius: 9999,
  ...
  showcaseScore: (hue: number | string): CSSProperties => ({
    display: "inline-flex",
    alignItems: "center",
    gap: 6,
    padding: "4px 10px",
  ...
  danmaku: (y: number, delay: number): CSSProperties => ({
    position: "absolute",
    top: `${y}%`,
    left: 0,
    padding: "4px 10px",
  ...
  cursorLabel: (visible: boolean, x: number, y: number): CSSProperties => ({
    position: "absolute",
    left: x,
    top: y,
...
function AnimatedWord({ children, delay, reduced }: AnimatedWordProps) {
...
function Showcase({ poster, lang, hero }: ShowcaseProps) {
  const [cursor, setCursor] = useState<CursorState>({ x: 0, y: 0, on: false });
  ...
  const trendingFallback = hero.trendingFallback;
  ...
  return (
    ...
    >
      ...
      {danmaku.map((d, i) => (
        <span key={i} data-danmaku="" style={s.danmaku(d.y, d.delay)}>
          {d.text}
        </span>
      ))}
...
export default function HeroSection({ dict, poster }: HeroSectionProps) {
  ...
  const titleLine1 = hero.titleLine1;
  const titleLine2 = hero.titleLine2;
  const period = hero.period;
  const line1: readonly string[] = Array.isArray(titleLine1)
    ? titleLine1
    : [titleLine1];
  const line2: readonly string[] = Array.isArray(titleLine2)
    ? titleLine2
    : [titleLine2];
  const sep = lang === "en" ? " " : "";
      ...
      <div className="container">
        <div className="hero-grid" style={s.grid}>
          <div>
            ...
            >
              <Link href="/" className="hero-btn-primary" style={s.btnPrimary}>
                <span style={s.btnPrimaryLabel} aria-hidden="true">
                  [ START ]
                </span>
                <span style={s.btnPrimaryMain}>
                  {hero.ctaPrimary}
                  <span className="hero-btn-arrow" style={s.btnPrimaryArrow} aria-hidden>
                    {"→"}
                  </span>
                </span>
              </Link>
            ...
            <div style={s.metaRow}>
              ...
              <span>{hero.metaSources}</span>
              <span style={s.metaSep}>·</span>
              <span>{hero.metaDaily}</span>
            </div>
          </div>
          ...
          >
            <Showcase poster={poster} lang={lang} hero={hero} />
          </Motion.div>
        </div>
      </div>
    </section>
