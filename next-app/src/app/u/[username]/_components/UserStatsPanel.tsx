...
function DonutChart({ counts, total }: { counts: Partial<Record<string, number>>; total: number }) {
  if (total === 0) {
    return (
...
export default function UserStatsPanel({ watching, lang: _lang }: UserStatsPanelProps) {
  ...
  let topSeasonLabel: string | null = null;
  if (topSeasonKey) {
    const [season, year] = topSeasonKey.split("-");
    // Remove emoji suffixes from the season label (❄️🌸☀️🍂)
  ...
  return (
    ...
    >
          ...
          (s) =>
            (counts[s] ?? 0) > 0 && (
              <div key={s} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ color: "#ffffff", fontSize: 12, fontWeight: 700, marginLeft: 2 }}>
                  {counts[s]}
                </span>
              </div>
            ),
      ...
      <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
        {totalEps > 0 && (
          <div>
            <div style={{ color: "#ffffff", fontSize: 22, fontWeight: 800, lineHeight: 1 }}>
              {totalEps}
            </div>
            <div style={{ color: "rgba(235,235,245,0.45)", fontSize: 11, marginTop: 3 }}>
              {t("social.statsEpisodes")}
            </div>
          </div>
        )}
        {topGenres.length > 0 && (
          <div>
            ...
            <div style={{ color: "rgba(235,235,245,0.45)", fontSize: 11 }}>
              {t("social.statsGenres")}
            </div>
          </div>
        )}
        {topSeasonLabel && (
          <div>
            ...
            <div style={{ color: "rgba(235,235,245,0.45)", fontSize: 11, marginTop: 2 }}>
              {t("social.statsSeason")}
            </div>
          </div>
        )}
      </div>
