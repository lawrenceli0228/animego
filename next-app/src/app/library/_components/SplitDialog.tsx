...
function seasonLabel(season: SeasonLike): string {
...
const checkboxStyle = (selected: boolean): React.CSSProperties => ({
  ...
  height: 14,
  border: `1px solid ${
    selected ? `oklch(72% 0.15 ${HUE})` : `oklch(46% 0.06 ${HUE} / 0.55)`
  }`,
...
const buttonStyle = (
  ...
  background: primary
    ...
    : "transparent",
  border: `1px solid oklch(${primary ? "62% 0.17" : "46% 0.06"} ${HUE} / ${
    disabled ? "0.30" : "0.55"
  })`,
  borderRadius: 3,
  color: primary
      ...
      >
            ...
            {seasons.map((sn) => {
              ...
              return (
                <button
                  ...
                  style={seasonStyle(isSel)}
                  onClick={() => toggle(sn.id)}
                  aria-pressed={isSel}
                >
                  <span style={checkboxStyle(isSel)} aria-hidden />
        ...
        <div style={s.footer}>
          ...
          <button
            ...
            type="button"
            style={buttonStyle(true, confirmDisabled)}
