...
type RowState = "pending" | "running" | "matched" | "ambiguous" | "failed";

// Generic event shape mirrors lib/library/types.js — kept loose to dodge a
// 638-line type rewrite. The aggregator only reads kind / clusterKey /
// total / verdict / error.
 ...
 */
export function aggregateEvents(events: ImportEvent[]): {
  rows: AggregatedRow[];
  total: number;
  finished: number;
} {
  ...
  for (const e of events) {
    ...
    if (e.kind === "clusterStart" && e.clusterKey) {
      map.set(e.clusterKey, {
        clusterKey: e.clusterKey,
        total: e.total ?? 0,
        state: "running",
      });
    } else if (e.kind === "clusterDone" && e.clusterKey) {
      const cur = map.get(e.clusterKey) ?? {
        clusterKey: e.clusterKey,
        total: 0,
 ...
 */
function groupRows(rows: AggregatedRow[]) {
  const buckets: Record<RowState, AggregatedRow[]> = {
    running: [],
    ambiguous: [],
    failed: [],
    matched: [],
  ...
  return GROUP_ORDER.map((state) => ({ state, rows: buckets[state] })).filter(
    (g) => g.rows.length > 0,
  );
}
...
export function ImportDrawer({
  ...
  const showDismiss =
    (status === "done" || status === "error") && typeof onDismiss === "function";
  ...
  const showBackground =
    status === "running" && typeof onDismiss === "function";
  ...
  const stats = useSummary
    ...
    : rows.reduce(
        ...
        { matched: 0, running: 0, ambiguous: 0, failed: 0, pending: 0 } as Record<
          RowState,
          number
        >,
      );
        ...
        <div style={s.header}>
          ...
          <div style={s.meter}>
            ...
            <div style={s.meterBar}>
              <div style={meterFillStyle(meterPct)} />
            </div>
                  ...
                  {groupRowsList.map((row) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          aria-hidden
                          style={rowIconStyle(meta.color, meta.bg, isRunning)}
                        >
                          {meta.glyph}
                        </div>
                        <div style={s.rowInfo}>
...
const meterFillStyle = (pct: number): React.CSSProperties => ({
  height: "100%",
...
const rowIconStyle = (
  ...
  flexShrink: 0,
  ...(animate
    ? { animation: "animego-import-pulse 1.2s ease-in-out infinite" }
    : null),
});
