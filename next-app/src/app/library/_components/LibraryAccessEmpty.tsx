...
// Ported from client/src/components/library/LibraryAccessEmpty.jsx.
...
const s = {
  ...
  btn: (hover: boolean, primary: boolean): CSSProperties => ({
    background: primary
      ? hover
      ...
      : hover
        ...
        : "transparent",
    border: `1px solid oklch(${primary ? 62 : 46}% ${primary ? 0.19 : 0.06} ${HUE} / ${primary ? (hover ? 0.85 : 0.65) : hover ? 0.65 : 0.4})`,
    borderRadius: 2,
    padding: "10px 18px",
    fontFamily: "'JetBrains Mono',monospace",
    fontSize: 11,
...
export type EmptyKind = "loading" | "missing" | "error" | "denied";
...
function HudButton({ onClick, label, primary, testId }: HudButtonProps) {
...
interface LibraryAccessEmptyProps {
  kind: EmptyKind;
  onReauthorize?: () => void;
  onRetry?: () => void;
...
export function LibraryAccessEmpty({
  kind,
  onReauthorize,
  onRetry,
  onBackToLibrary,
}: LibraryAccessEmptyProps) {
  const { t } = useLang();
  ...
  return (
    <div style={s.wrapper} data-testid="library-access-empty" data-kind={kind}>
      ...
      <div
        style={{ ...s.eyebrow, ...(kind === "denied" ? s.eyebrowDenied : null) }}
      >
        {eyebrow}
      </div>
