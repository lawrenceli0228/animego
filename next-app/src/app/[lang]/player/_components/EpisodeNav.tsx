...
const epHue = (ep: number | null | undefined) =>
  ep != null && Number.isFinite(ep) ? (210 + (ep as number) * 10) % 360 : 210;
...
const s = {
  ...
  chip: (active: boolean, hue: number): CSSProperties => ({
    position: "relative",
    padding: "8px 14px",
    borderRadius: 2,
    border: active
 ...
 */
function EpisodeNav({ episodes, currentEpisode, onSelect }: EpisodeNavProps) {
      ...
      {episodes.map((ep, i) => {
        ...
        const chipMotion = reduced
          ...
          : {
              ...
              transition: { duration: 0.2, delay: 0.04 * i, ease: "easeOut" as const },
            };
