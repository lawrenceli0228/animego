...
interface WatchRhythmStripProps {
  rhythm: WatchRhythm;
  compact?: boolean;
}
...
const s = {
  ...
  cell: (filled: boolean, today: boolean): React.CSSProperties => ({
    width: 14,
    height: 14,
    borderRadius: 3,
    background: filled ? "#0a84ff" : `oklch(20% 0.04 ${HUE} / 0.55)`,
 ...
 */
function WatchRhythmStrip({
  rhythm,
  compact = false,
}: WatchRhythmStripProps) {
  const { t } = useLang();
  ...
  return (
    ...
    >
      ...
      <div style={s.stat}>
        <span style={s.num} data-testid="rhythm-this-week">
          {rhythm.thisWeek}
        </span>
      ...
      <div style={s.stat}>
        <span style={s.num} data-testid="rhythm-streak">
          {rhythm.streak}
        </span>
      ...
      <div
        ...
        aria-label={t("library.rhythm.cal14Aria").replace(
          "{{days}}",
          String(rhythm.totalDays),
        )}
      >
...
export { WatchRhythmStrip };
export default WatchRhythmStrip;
