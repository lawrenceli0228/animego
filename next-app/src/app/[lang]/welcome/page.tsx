...
export async function generateMetadata({
  ...
  const [{ locale, dict, lang }, frierenDetail, trending] = await Promise.all([
  ...
  return {
    ...
    openGraph,
    twitter,
    alternates: buildAlternates("/welcome", locale),
  };
}

export default async function LandingPage({
  params,
}: PageProps<"/[lang]/welcome">) {
  // `lang` and `dict` come from the one route-param resolution and are handed
  // to the landing sections as sibling props. Those sections used to infer
  // their language by comparing dictionary content against a Chinese sentinel
  // ('。', '放送中'), which a third language matches none of and so silently
  // resolved to zh or en. Taking both from resolveLocale makes dict/lang
