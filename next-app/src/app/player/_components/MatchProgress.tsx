...
export type MatchStepStatus = "pending" | "active" | "done" | "fail";

const STATUS_COLOR: Record<MatchStepStatus, string> = {
...
const s = {
  ...
  steps: { position: "relative" } as CSSProperties,
  ...
  step: (status: MatchStepStatus): CSSProperties => ({
    position: "relative",
    display: "flex",
    alignItems: "center",
    gap: 14,
  ...
  stepNum: (status: MatchStepStatus): CSSProperties => ({
    ...mono,
    fontSize: 10,
    width: 28,
    flexShrink: 0,
  ...
  stepIcon: (status: MatchStepStatus): CSSProperties => ({
    width: 24,
    textAlign: "center",
    fontSize: 16,
    flexShrink: 0,
  ...
  stepText: (status: MatchStepStatus): CSSProperties => ({
    ...mono,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: "0.14em",
...
const STATUS_ICON: Record<MatchStepStatus, string> = {
 ...
 */
function StepIcon({ status }: { status: MatchStepStatus }) {
 ...
 */
function StepRow({ index, status, label, detail }: StepRowProps) {
  ...
  return (
    ...
    >
      ...
      <span style={s.stepNum(status)} aria-hidden>
        {numStr}
      </span>
...
export interface MatchProgressProps {
  fileCount: number;
  keyword: string;
  // Accept either a 1|2|3-keyed object (subagent C's StepStatus) or a generic
  // numeric record — useful for ad-hoc callers / tests.
  stepStatus:
    | Record<number, MatchStepStatus>
    | { 1: MatchStepStatus; 2: MatchStepStatus; 3: MatchStepStatus };
  onClear: () => void;
}
...
function MatchProgress({
  ...
  const steps = [
    {
      key: 1,
      label: t("player.stepParse"),
      detail: `${fileCount} ${t("player.videos")}, ${t("player.keyword")}: "${keyword}"`,
    },
  ...
  return (
    <div style={s.container}>
      ...
      <CornerBrackets inset={6} size={8} opacity={0.3} />
      ...
      <div style={s.steps}>
        ...
        {steps.map(({ key, label, detail }, i) => {
          const status =
            (stepStatus as Record<number, MatchStepStatus>)[key] || "pending";
          return (
            <StepRow
              key={key}
              index={i + 1}
