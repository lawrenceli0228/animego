 ...
 *
 * P6.6 port note: this file is // @ts-nocheck because libass's dynamic
 * font / ASS parser shapes don't type narrowly without rewriting the
 * whole module. JSDoc retained for IDE hints.
 */
...
// served raw as a module worker. The legacy Vite project used
// `?worker&url` to bundle it; the Next 16 / webpack equivalent is to
// pass a URL constructed with `import.meta.url` from the npm package
// — webpack detects this pattern and emits a self-contained worker
// bundle next to the main chunk.
//
// WASM + default.woff2 are static binaries served from public/jassub/
// (copied during P6.5) because webpack does not reliably expose
// node_modules subpaths with the right Content-Type under COEP.
