 ...
 *
 * Dev-mode gate: PlayerShell mounts this only when localStorage['animego.devMode']
 * is set (matches legacy gating). Component is otherwise a no-op render guard.
 *
 * Ported from legacy client/src/components/player/HeatmapTuner.jsx (P6.6).
 */

import { useCallback, useEffect, useRef, useState, type CSSProperties } from "react";
import { mono } from "@/components/landing/shared/hud-tokens";
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore — ts-nocheck'd module: artplayer plugin shape is dynamic + untyped
import { applyHeatmapPath } from "@/lib/heatmapPath";

const STORAGE_KEY = "animego.heatmapConfig";

interface GroupAValues {
  bandHeight: number;
  scaleY: number;
  fillColor: string;
  fillOpacity: number;
...
const GROUP_A_DEFAULTS: GroupAValues = {
  bandHeight: 2,
  scaleY: 17.35,
  fillColor: "#ffffff",
  fillOpacity: 0.4,
...
const GROUP_B_DEFAULTS: GroupBValues = {
  sampling: 7,
  smoothing: 0.35,
  flattening: 0.05,
  scale: 0.011,
...
const GROUP_A_RANGES: Record<keyof Omit<GroupAValues, "fillColor" | "alwaysVisible">, Range> = {
  bandHeight: { min: 0, max: 80, step: 1 },
  scaleY: { min: 0.2, max: 30, step: 0.05 },
  fillOpacity: { min: 0.05, max: 1, step: 0.05 },
};

const GROUP_B_RANGES: Record<keyof GroupBValues, Range> = {
...
function loadGroupB(): GroupBValues {
  try {
...
function persistGroupB(values: GroupBValues): void {
  try {
...
function applyGroupA(values: GroupAValues): void {
...
function applyGroupBLive(values: GroupBValues): ApplyDiag {
  /* eslint-disable @typescript-eslint/no-explicit-any */
  const art =
    typeof window !== "undefined" ? (window as any).__artInstance : null;
  if (!art) {
    warnOnce("no artplayer instance on window.__artInstance");
    return { ok: false, reason: "no art instance" };
  }
  const result = applyHeatmapPath(art, values) as ApplyDiag;
...
function Slider({ label, valueKey, value, range, onChange }: SliderProps) {
  return (
    <div style={s.row}>
...
export function HeatmapTuner() {
  const [collapsed, setCollapsed] = useState(false);
  const [groupA, setGroupA] = useState<GroupAValues>(() => ({
    ...GROUP_A_DEFAULTS,
  }));
  const [groupB, setGroupB] = useState<GroupBValues>(() => loadGroupB());
  const [copied, setCopied] = useState(false);
  const [diag, setDiag] = useState<ApplyDiag | null>(null);
  ...
  const updateA = useCallback(
    <K extends keyof GroupAValues>(key: K, val: GroupAValues[K]) => {
      setGroupA((prev) => ({ ...prev, [key]: val }));
    },
    [],
  );
  const updateB = useCallback(
    <K extends keyof GroupBValues>(key: K, val: GroupBValues[K]) => {
      ...
      {!collapsed && (
        <div style={s.body}>
          ...
          <Slider
            label="bandHeight"
            valueKey="bandHeight"
            value={groupA.bandHeight}
            range={GROUP_A_RANGES.bandHeight}
            onChange={(v) => updateA("bandHeight", v)}
          />
          <Slider
            label="scaleY"
            valueKey="scaleY"
            value={groupA.scaleY}
            range={GROUP_A_RANGES.scaleY}
            onChange={(v) => updateA("scaleY", v)}
          />
          <div style={s.row}>
            <div style={s.rowHeader}>
          ...
          <Slider
            label="fillOpacity"
            valueKey="fillOpacity"
            value={groupA.fillOpacity}
            range={GROUP_A_RANGES.fillOpacity}
            onChange={(v) => updateA("fillOpacity", v)}
          />
          ...
          <Slider
            label="sampling"
            valueKey="sampling"
            value={groupB.sampling}
            range={GROUP_B_RANGES.sampling}
            onChange={(v) => updateB("sampling", v)}
          />
          <Slider
            label="smoothing"
            valueKey="smoothing"
            value={groupB.smoothing}
            range={GROUP_B_RANGES.smoothing}
            onChange={(v) => updateB("smoothing", v)}
          />
          <Slider
            label="flattening"
            valueKey="flattening"
            value={groupB.flattening}
            range={GROUP_B_RANGES.flattening}
            onChange={(v) => updateB("flattening", v)}
          />
          <Slider
            label="scale"
            valueKey="scale"
            value={groupB.scale}
            range={GROUP_B_RANGES.scale}
            onChange={(v) => updateB("scale", v)}
          />
          <Slider
            label="minHeight"
            valueKey="minHeight"
            value={groupB.minHeight}
            range={GROUP_B_RANGES.minHeight}
            onChange={(v) => updateB("minHeight", v)}
          />
