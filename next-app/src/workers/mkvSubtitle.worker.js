...
// Next-app port: legacy SPA prepended pako_inflate via Vite `?raw` blob
// concat (resolveSubtitle.js → createMkvWorker). Next webpack bundles
// `import` statements inside module workers cleanly, so we just import
// pako directly here and expose its inflate on self.pako so the rest of
// the worker body (still expecting `self.pako.inflate`) keeps working
// verbatim. pako.inflate is synchronous — much faster than per-block
